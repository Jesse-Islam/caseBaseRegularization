...
```{r}
highDimSurvData=as.data.frame(na.omit(lusc.rnaseq2$merged.dat))

highDimNames=colnames(highDimSurvData)

modelSplinesregularized<- fitSmoothHazard(status~bs(OS)+.-bcr , data = highDimSurvData, ratio = 100, time ='OS')

model4 <- fitSmoothHazard(status ~ ., data = ERSPC, ratio = 100)
```
